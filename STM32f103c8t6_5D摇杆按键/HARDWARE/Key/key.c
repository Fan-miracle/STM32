...
void Key_Init(void)
{
	...
	GPIO_Str.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4;
...
{	 
	...
	if(key_up&&(KEY_UP==0||KEY_DWN==0||KEY_LFT==0||KEY_RHT==0||KEY_MID==0||KEY_SET==0||KEY_RST==0))		//按键按下
	{
		delay_ms(20);//去抖动 
		key_up=0;
	...
	else if(KEY_UP==1&&KEY_DWN==1&&KEY_LFT==1&&KEY_RHT==1&&KEY_MID==1&&KEY_SET==1&&KEY_RST==1)				//按键回正
	{
